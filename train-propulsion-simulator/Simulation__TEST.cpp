...
int main() {
	...
	*/
	...
	// Simulation
	std::string inputFileAbsolutePath_SIMULATION = "C:/Users/Leith/Desktop/MySimulation__test.txt";
	InputFileReader_Simulation* inputFileReader_Simulation_SIMULATION = new InputFileReader_Simulation(inputFileAbsolutePath_SIMULATION);
	inputFileReader_Simulation_SIMULATION->userDefinedTracks.push_back(inputFileReader_Simulation_TRACK->userDefinedTracks[0]);
	inputFileReader_Simulation_SIMULATION->userDefinedCouplers.push_back(inputFileReader_Simulation_COUPLER->userDefinedCouplers[0]);
	inputFileReader_Simulation_SIMULATION->userDefinedCars.push_back(inputFileReader_Simulation_CAR->userDefinedCars[0]);
	inputFileReader_Simulation_SIMULATION->userDefinedLocomotives.push_back(inputFileReader_Simulation_LOCOMOTIVE->userDefinedLocomotives[0]);
	inputFileReader_Simulation_SIMULATION->userDefinedLocomotiveOperators.push_back(inputFileReader_Simulation_LOCOMOTIVE_OPERATOR->userDefinedLocomotiveOperators[0]);
	inputFileReader_Simulation_SIMULATION->userDefinedTrainConsists.push_back(inputFileReader_Simulation_TRAIN_CONSIST->userDefinedTrainConsists[0]);
	inputFileReader_Simulation_SIMULATION->userDefinedSimulations.push_back(new Simulation(inputFileReader_Simulation_SIMULATION));
	std::string validString_SIMULATION = inputFileReader_Simulation_SIMULATION->userDefinedSimulations[0]->load();
	std::cout << "Line " << inputFileReader_Simulation_SIMULATION->currentLineNumber << ": " << validString_SIMULATION << std::endl;
	std::cout << std::endl;
	std::cout << "SIMULATION" << std::endl;
	std::cout << std::endl;
	inputFileReader_Simulation_SIMULATION->userDefinedSimulations[0]->console();
	std::cout << std::endl;
